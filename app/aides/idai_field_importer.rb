class IdaiFieldImporter
  ...
  def initialize(object, default_namespace_url, logger = Rails.logger, publish = true, verbose = false)
    ...
    unless @file.is_a?(File) || @file.is_a?(Array)
      raise "IdaiFieldSKOSImporter#import: Parameter 'file' should be a File or an Array."
    end
    ...
    @iqvoc_origins = {} # save origin generated by iQvoc for ids generated by iDAI.field script
  end
  ...
  def import(file)
    ...
    @logger.info 'IdaiFieldSKOSImporter: Importing triples...'
    file.each_with_index do |line, index|
      extracted_triple = *extract_triple(line)

      if @verbose && has_unknown_namespaces?(extracted_triple)
        @logger.warn "IdaiFieldSKOSImporter: Unknown namespaces. Skipping #{extracted_triple.join(' ')}"
      end

      unless has_valid_origin?(extracted_triple)
        @logger.warn "IdaiFieldSKOSImporter: Invalid origin. Skipping #{extracted_triple.join(' ')}"
        next
      end
  ...
  def import_first_level_objects(types, subject, predicate, object)
    if (predicate == 'rdf:type' && types[object] && subject =~ /^:(.+)$/)
      ...
      if is_concept && editorial_note
        ...
        @logger.info "IdaiFieldSKOSImporter: Subject with origin '#{concept.origin}' already exists. Skipping duplicate creation (should be no problem)."
      elsif (@existing_origins[origin])
        if (types[object] == @existing_origins[origin])
          @logger.info "IdaiFieldSKOSImporter: Subject with origin '#{origin}' already exists. Skipping duplicate creation (should be no problem)."
        else
          @logger.warn "IdaiFieldSKOSImporter: Subject with origin '#{origin} already exists but has another class (#{@existing_origins[origin]}) then the one I wanted to create (#{types[object]}). You seem to have a problem with your configuration!"
        end
      else
        @logger.info "IdaiFieldSKOSImporter: Creating Subject: #{subject} #{predicate} #{object}" if @verbose
        ...
        if is_concept
            ...
            @logger.info "IdaiFieldSKOSImporter: Created origin #{new_concept.origin} for #{origin}" if @verbose
        end
  ...
  def import_second_level_objects(types, final, line)
    ...
    unless subject
      if final
        @logger.warn "IdaiFieldSKOSImporter: Couldn't find Subject with origin '#{subject_origin}''. Skipping entry '#{subject} #{predicate} #{object}.'"
      else
    ...
    if (object =~ /^:(.*)$/ && types[predicate])
      ...
      unless object
        if final
          @logger.warn "IdaiFieldSKOSImporter: Couldn't find Object with origin '#{object_origin}'. Skipping entry ':#{subject_origin} #{predicate} #{object_origin}.'"
        else
          @unknown_second_level_triples << line
        end
        return false
    ...
    if predicate == "skosxl:prefLabel"
        ...
        if relation
            @logger.info "IdaiFieldSKOSImporter: Label relation does already exist. Skipping entry: '#{subject} #{predicate} #{object}.'"
            return false
        end
        ...
        if note
            note.destroy
            @logger.info "IdaiFieldSKOSImporter: Overwriting existing scope note for entry: '#{subject} #{predicate} #{object}.'"
        end
    end
