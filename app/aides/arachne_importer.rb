class ArachneImporter
  ...
  def initialize(object, default_namespace_url, logger = Rails.logger, publish = true, verbose = false)
    ...
    unless @file.is_a?(File) || @file.is_a?(Array)
      raise "ArachneImporter#import: Parameter 'file' should be a File or an Array."
    end
    ...
    @iqvoc_origins = {} # save origin generated by iQvoc for ids generated by Arachne script
  end
  ...
  def import(file)
    ...
    @logger.info 'ArachneImporter: Importing triples...'
    file.each_with_index do |line, index|
      extracted_triple = *extract_triple(line)

      if @verbose && has_unknown_namespaces?(extracted_triple)
        @logger.warn "ArachneImporter: Unknown namespaces. Skipping #{extracted_triple.join(' ')}"
      end

      unless has_valid_origin?(extracted_triple)
        @logger.warn "ArachneImporter: Invalid origin. Skipping #{extracted_triple.join(' ')}"
        next
      end
  ...
  def publish
    ...
    if @publish
      ...
      sorted_new_subjects.each do |origin, klass|
        ...
        subject.to_review = nil
        subject.published_version_id = nil
        subject.save!
        published += 1
      end
    end
  ...
  def import_first_level_objects(types, subject, predicate, object)
    if (predicate == 'rdf:type' && types[object] && subject =~ /^:(.+)$/)
      ...
      editorial_note = Note::SKOS::EditorialNote.find_by(value: "iDAI.objects origin: #{origin}")
      if is_concept && editorial_note
        concept = Concept::SKOS::Base.find(editorial_note.owner_id)
        @iqvoc_origins[origin] = concept.origin
        @logger.info "ArachneImporter: Subject with origin '#{concept.origin}' already exists. Skipping duplicate creation (should be no problem)."
      elsif (@existing_origins[origin])
        if (types[object] == @existing_origins[origin])
          @logger.info "ArachneImporter: Subject with origin '#{origin}' already exists. Skipping duplicate creation (should be no problem)."
        else
          @logger.warn "ArachneImporter: Subject with origin '#{origin} already exists but has another class (#{@existing_origins[origin]}) then the one I wanted to create (#{types[object]}). You seem to have a problem with your configuration!"
        end
      else
        @logger.info "ArachneImporter: Creating Subject: #{subject} #{predicate} #{object}" if @verbose

        notes = []
        if is_concept
            notes << Note::SKOS::EditorialNote.create(value: "iDAI.objects origin: " + origin, owner_type: types[object], language: "de")
        end

        new_concept = types[object].create do |new_object|
          new_object.origin = is_concept ? Origin.new().to_s : origin
          new_object.notes = notes
        end

        if is_concept
            @iqvoc_origins[origin] = new_concept.origin
            @logger.info "ArachneImporter: Created origin #{new_concept.origin} for #{origin}" if @verbose
        end
        @seen_first_level_objects[new_concept.origin] = types[object]
        @new_subjects[new_concept.origin] = types[object]
      end
      true
    else
      false
  ...
  def import_second_level_objects(types, final, line)
    ...
    subject_origin = @iqvoc_origins[$1] ? @iqvoc_origins[$1] : $1
    subject = load_first_level_object(subject_origin)
    unless subject
      if final
        @logger.warn "ArachneImporter: Couldn't find Subject with origin '#{subject_origin}''. Skipping entry '#{subject} #{predicate} #{object}.'"
      else
    ...
    if (object =~ /^:(.*)$/ && types[predicate])
      object_origin = @iqvoc_origins[$1] ? @iqvoc_origins[$1] : $1
      object = load_first_level_object(object_origin)
      unless object
        if final
          @logger.warn "ArachneImporter: Couldn't find Object with origin '#{object_origin}'. Skipping entry ':#{subject_origin} #{predicate} #{object_origin}.'"
        else
          @unknown_second_level_triples << line
        end
        return false
    ...
    if predicate == "skosxl:prefLabel"
        concept_id = Concept::SKOS::Base.find_by(origin: subject_origin).id
        label_id = Label::SKOSXL::Base.find_by(origin: object_origin).id
        relation = Labeling::SKOSXL::PrefLabel.find_by(owner_id: concept_id, target_id: label_id)
        if relation
            @logger.info "ArachneImporter: Label relation does already exist. Skipping entry: '#{subject} #{predicate} #{object}.'"
            return false
        end
    elsif predicate == "skos:scopeNote"
        concept_id = Concept::SKOS::Base.find_by(origin: subject_origin).id
        language = object.split("\"@").last
        note = Note::SKOS::ScopeNote.find_by(owner_id: concept_id, language: language)
        if note
            note.destroy
            @logger.info "ArachneImporter: Overwriting existing scope note for entry: '#{subject} #{predicate} #{object}.'"
        end
    end
